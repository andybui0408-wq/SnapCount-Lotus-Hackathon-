...
export async function getDailyBriefing(): Promise<string> {
  ...
  if (scans.length === 0) return "No data yet. Scan your inventory to start tracking.";
  ...
  const prompt = `You are a friendly store inventory assistant. Be conversational and brief.
...
Write a SHORT briefing in English with EXACTLY this format — each item on its own line:

Line 1: One short sentence — total items and how many products need restocking
Line 2: (blank)
${criticalItems.length > 0 ? 'For each critical product: "⚠️ [Product]: [X] left — restock immediately"\n' : ""}${urgentItems.length > 0 ? 'For each low-stock product: "📦 [Product]: [X] left — consider restocking"\n' : ""}Line: (blank)
