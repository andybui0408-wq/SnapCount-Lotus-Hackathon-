...
| Charts | None needed — stock bars are pure CSS | — |
...
### AI Daily Briefing prompt (Vietnamese):

This is the MOST IMPORTANT prompt. It replaces charts and tables with a plain-language Vietnamese briefing that any shop owner can understand instantly. It runs every time the user opens the Dashboard tab, using the latest scan data and trends.

```typescript
// src/services/dailyBriefing.ts

const DAILY_BRIEFING_PROMPT = `You are an AI inventory assistant for a Vietnamese convenience store owner. Speak to them directly in Vietnamese, warmly and conversationally — like a trusted employee giving a morning report.
...
Example tone:
"Anh ơi, hôm nay Monster Energy chỉ còn 1 lon và Bánh mì Kinh Đô còn 1 gói — hai món này hết nội trong ngày mai. Coca-Cola bán nhanh nhất tuần, giảm 19 lon trong 7 ngày. Nên gọi đặt hàng trước thứ Bảy, nếu không Chủ nhật sẽ hết Trà xanh C2 và Fanta luôn."`;
```

### Revenue estimation logic (runs in browser, no Gemini needed):

```typescript
// src/services/revenueEstimator.ts

interface ProductPrice {
  name: string;
  sellPrice: number;  // VND per unit — set once by user
}

// Stored in localStorage key: "countr_prices"
...
function estimateRevenue(
  ...
  for (const [product, history] of Object.entries(trends)) {
    if (history.length < 2) continue;
    const first = history[0].count;
    const last = history[history.length - 1].count;
    const unitsSold = Math.max(0, first - last); // stock dropped = units sold
...
User sets sell prices ONCE via a simple modal (product name + price input per item). Prices stored in localStorage. Revenue recalculates automatically from scan trend data — units sold = stock decrease between scans.

### Order deadline calculation (runs in browser):

```typescript
// src/services/orderDeadlines.ts

interface OrderDeadline {
  product: string;
  currentStock: number;
  avgPerDay: number;
  runsOutDate: Date;      // projected stockout date
  orderByDate: Date;       // runsOutDate minus 1 day (delivery buffer)
  orderByLabel: string;    // "Thứ Bảy 22/03" — Vietnamese weekday
...
function calculateDeadlines(
  depletionData: { product: string; current: number; avgPerDay: number }[]
): OrderDeadline[] {
  const now = new Date();
  ...
  return depletionData
    ...
    .map(d => {
      const daysLeft = d.current / d.avgPerDay;
...
│   │   ├── SummaryCards.tsx
│   │   ├── ResultsList.tsx
│   │   ├── AIBriefingCard.tsx       # Vietnamese daily briefing from Gemini
│   │   ├── StockBars.tsx            # Visual battery-style stock bars
│   │   ├── OrderDeadlines.tsx       # "Order by Friday" deadline cards
│   │   ├── RevenueStrip.tsx         # Weekly revenue + top seller
│   │   ├── PriceEditor.tsx          # Modal: set sell prices per product
...
│   │   ├── geminiAPI.ts
│   │   ├── geminiAnalysis.ts
│   │   ├── dailyBriefing.ts         # Gemini Vietnamese briefing
│   │   ├── revenueEstimator.ts      # Revenue calc from trends + prices
│   │   ├── orderDeadlines.ts        # "Order by" date calculation
│   │   ├── mergeResults.ts
│   │   ├── scanHistory.ts
│   │   ├── restockAgent.ts
│   │   ├── zaloMessenger.ts
...
### 2. Dashboard Page — "Intelligence" (NO charts, NO tables)

The dashboard does NOT show line charts or spreadsheet tables. Instead it shows AI-generated intelligence that any shop owner understands instantly.

**Layout top to bottom:**

**A) AI Daily Briefing card** (the hero element)
- Full width card, bg var(--off-white), border-left 3px solid var(--black)
- Padding 20px
- Small label at top: "AI BRIEFING" in JetBrains Mono 10px uppercase, var(--stone)
- Below: Gemini's Vietnamese language briefing text, Outfit 15px / 400, line-height 1.6
- The briefing is 3-5 sentences of conversational Vietnamese telling the owner what's urgent
- Generated by calling Gemini with DAILY_BRIEFING_PROMPT on page load
- Show a subtle loading skeleton while Gemini responds
- Cache the briefing in localStorage for 1 hour so it doesn't re-call on every tab switch

**B) Order deadline cards** (most urgent first)
- Section label: "Đặt hàng trước" (Order before) in Syne 18px / 700
- List of deadline cards, each showing:
  ```
  ┌───────────────────────────────────┐
  │ Monster Energy          ĐẶT NGAY │
  │ Còn 1 lon · Hết: Chủ nhật 23/03  │
  └───────────────────────────────────┘
  ```
  - Product name: Outfit 15px / 600
  - Deadline pill (right-aligned):
    - "ĐẶT NGAY" (order now) = bg var(--black), text white, JetBrains Mono 10px bold
    - "Ngày mai" (tomorrow) = border var(--charcoal), JetBrains Mono 10px
    - "Thứ Bảy 22/03" = JetBrains Mono 10px, var(--stone)
  - Second line: "Còn {n} {unit}" + "Hết: {weekday} {date}" in Outfit 13px, var(--charcoal)
  - Card: bg var(--white), border 1px solid var(--border), border-radius var(--radius-md), padding 14px
  - Only show products with urgency "today", "tomorrow", or "this_week". Hide "safe" items.

**C) Visual stock bars** (replace the depletion table)
- Section label: "Tồn kho" (Stock) in Syne 18px / 700
- Each product as a horizontal bar:
  ```
  Coca-Cola can                    5
  ██████░░░░░░░░░░░░░░░░░░░  21%
  ...
  ████████████████████░░░░░  50%
  ```
  - Product name: Outfit 14px / 500, left-aligned
  - Current qty: JetBrains Mono 14px / 700, right-aligned on same line
  - Bar below: full width, 8px height, border-radius 4px
    - Background track: var(--off-white)
    - Fill: var(--black) normally, opacity varies by fill level
      - ≤ 15% fill: solid var(--black) (critical — stands out)
      - 15-40% fill: var(--charcoal)
      - > 40% fill: var(--stone) (healthy — subtle)
    - Width: `(current / maxHistorical) * 100%` — where maxHistorical is the highest count ever recorded for this product
  - Sorted by fill% ascending (most empty first)
  - Tapping a bar could expand to show more detail (optional)

**D) Revenue estimate strip** (if user has set sell prices)
- Only shows if user has configured prices in the price editor
- Horizontal strip, 2-3 columns:
  ```
  ┌─────────────┬─────────────┬─────────────┐
  │  2.450.000đ │   350.000đ  │ Coca-Cola   │
  │  TUẦN NÀY   │   TRUNG BÌNH│ BÁN CHẠY    │
  │             │    / NGÀY   │ NHẤT        │
  └─────────────┴─────────────┴─────────────┘
  ```
  - Numbers: JetBrains Mono 22px / 700
  - Labels: JetBrains Mono 9px uppercase, var(--stone), letter-spacing 1.5px
  - Background: var(--off-white), border: 1px solid var(--border)
  - If no prices set, show a subtle prompt: "Nhập giá bán để xem doanh thu →" with tap to open price editor

**E) "Email Report" button**
- Full width, 52px, bg var(--black), text white
- At the bottom of the page

**F) Price editor modal** (accessed from revenue strip or Profile tab)
- List of all known products from scan history
- Each row: product name (left) + VND price input (right)
- Input: JetBrains Mono 14px, 100px wide, right-aligned
- Placeholder: "0đ"
- Save to localStorage key "countr_prices"
- Only needs to be set once — persists forever
...
Demo sell prices (pre-seeded in localStorage key "countr_prices"):
- Coca-Cola can: 10.000đ
- Fanta Orange bottle: 10.000đ
- Mì Hảo Hảo: 5.000đ
- Nước mắm Chin-Su: 25.000đ
- Trà xanh C2: 8.000đ
- Monster Energy: 22.000đ
- Bia Tiger: 15.000đ
- Bánh mì Kinh Đô: 12.000đ
...
- localStorage keys: "countr_scans", "countr_catalog", "countr_suppliers", "countr_prices", "countr_briefing_cache"
- AI briefing cached for 1 hour (store timestamp + text in countr_briefing_cache)
- All data text in JetBrains Mono
- Stock bars are pure CSS (no chart library) — width% = current/max × 100
- Revenue only shows if user has set prices in countr_prices
- Order deadlines = (current ÷ avgPerDay) days from today, minus 1 day for delivery
- Zalo: https://zalo.me/PHONE
- VND formatting: `amount.toLocaleString("vi-VN") + "đ"`
- Dates: DD/MM format, weekdays in Vietnamese

## Commands
```bash
npm create vite@latest countr-web -- --template react-ts
cd countr-web && npm install @emailjs/browser && npm run dev
```
